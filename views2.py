...
class Camera(nn.Module):
    def __init__(self):
        ...
        self.rot = nn.Parameter(torch.eye(3)[:-1])
        # self.register_buffer("R", torch.eye(3))
        
        self.t = nn.Parameter(torch.randn(3))
    ...
    def convert_to_rot_matrix(self):
        ...
        r2_proj = v2 - (v2 * r1).sum() * r1
        r2 = r2_proj / r2_proj.norm()

        r3 = torch.cross(r1, r2, dim=-1)

        self.R = torch.stack([r1, r2, r3], dim=-1)
    
    def forward(self, K, X):

        self.convert_to_rot_matrix()

        x_prime = torch.matmul(K,
                            torch.matmul(self.R, X) + self.t.unsqueeze(-1))
...
class GlobalOptimization(nn.Module):
    def __init__(self, x, x_prime):
        ...
        self.register_buffer("x_prime", torch.tensor(x_prime, dtype = torch.float32))

        self.focal = nn.Parameter(torch.rand(1) + 2)

        # self.register_buffer("K", torch.eye(3))
        # self.K = torch.diag_embed(torch.tensor([self.focal, self.focal, 1.])).to(device)

        self.cameras = Camera()

        # self.depth = nn.Parameter(torch.ones(x.shape[0]))
        self.depth = self.triangulate()[..., -1]
    ...
    def forward(self):
        ...
        # self.d = self.depth.abs()
        
        self.set_K()

        # self.K = torch.diag_embed([self.focal, self.focal, 1.])

        X = self.depth.abs() * torch.matmul(torch.linalg.inv(self.K), self.homogenize(self.x).t())
        x_prime = self.cameras(self.K, X)
        return ((x_prime - self.x_prime) ** 2).mean()
    
    def set_K(self):
        self.K = torch.eye(3, device = self.focal.device)
        self.K[0, 0] = self.focal.abs()
        self.K[1, 1] = self.focal.abs()
