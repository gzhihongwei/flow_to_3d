...
class Views(nn.Module):
    ...
    def __init__(self, args, video) -> None:
        ...
        self.frames = self.read_video_decord(video)[20:25] # TODO: Remove this!
        self.H, self.W = self.frames.shape[-2:]
        ...
        self.x, self.x_prime = self.get_correspondences(self.flow)
    ...
    def skew_symmetric_matrix(self, v):
        ...
        matrix = torch.zeros((*v.shape[:-1], 3, 3))
    ...
    def fundamental_matrix(self):
        ...
        S_zero = torch.zeros((U.size(0), 3, 3))
        S_zero[:, 0, 0] = S[:, 0]
        S_zero[:, 1, 1] = S[:, 1]
    ...
    def point_loss(self):
        ...
        P1 = torch.zeros((1, 3, 4))
        P1[:, :3, :3] = self.K

        P2_norm = torch.zeros((self.num_frames - 2, 3, 4))
        P2_norm[:, :3, :3] = self.R[:-1]
        P2_norm[:, :3, 3] = self.trans[:-1]
        P2 = torch.matmul(self.K, P2_norm)

        P3_norm = torch.zeros((self.num_frames - 2, 3, 4))
        P3_norm[:, :3, :3] = self.R[1:]
        P3_norm[:, :3, 3] = self.trans[1:]
        P3 = torch.matmul(self.K, P3_norm)
        ...
        next_correspondences = self.x_prime[:-1] + next_flow
        next_correspondences[..., -1] = 1
