...
def main(args):
    ...
    for i in frame_inds:
        X, R1_, R2_, t1_, t2_ = reconstruct_3_frames(K, video, i, flow_model, args)
        if i == 0:
            R1, R2, t1, t2 = R1_, R2_, t1_, t2_
        T = np.eye(4)
        T[:3, :3] = R1_
        T[:3, 3] = t1_
        X_all.append(X)
    ...
    # Visualize:
    geometries = visualized_3d_3frames(np.concatenate(X_canonicalized, axis=0), R1, t1, R2, t2)
